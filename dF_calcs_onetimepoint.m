...
figure(1)
hold on
bar([-av_dA(tp); -av_dB(tp); -av_dC(tp); -av_dD(tp)])
errorbar([1:4], [-av_dA(tp); -av_dB(tp); -av_dC(tp); -av_dD(tp)], [sd_A(tp); sd_B(tp); sd_C(tp); sd_D(tp)])
xlabel("Treatments: 0.05 mM, 0.5 mM, 5 mM, 0 mM")
ylabel("Activity as dF/dt (AU/s)")
title("Activity of TEV protease at differing concentrations with BS-TEV; /n 1-minute cumulative averages over brief time series")
%~~~~
tp = 50
figure(2)
hold on
